...
*	Modified Lastmod : Jan 2013
*	Modified Version : 1.7.0 (php5)
*
...
class Mail
{
	const priority_xpr = 0x01;
	const priority_msm = 0x02;
	const priority_imp = 0x04;
	const priority_full = 0x07; // must correspond to OR of all previous priority consts
	...
	protected $priority_use = self::priority_xpr;
	protected $priority_value = 3;
	...
	public function Priority ($priority = 3, $mask = null) {
		if (!array_key_exists($priority, $this->priorities)) {
			return false;
		}

		if ($mask !== null) {
			$this->UsePriority($mask);
		}
	...
	public function UsePriority ($mask) {
		if (!is_numeric($mask)) {
			return false;
		}
		
		// convert to integer, mask to the used bits only
		$this->priority_use = intval($mask) & self::priority_full;
		return true;
	}
	...
	protected function BuildMail () {
		...
		// set extended priority (new in 1.7.0)
		if ($this->priority_use & self::priority_xpr) {
			$this->xheaders['X-Priority'] = $this->priorities[$this->priority_value];
		}
		if ($this->priority_use & self::priority_msm) {
			$this->xheaders['X-MSMail-Priority'] = $this->priorities_ms[$this->priority_value];
		}
		if ($this->priority_use & self::priority_imp) {
			$this->xheaders['Importance'] = $this->priorities_ms[$this->priority_value];
		}

		$this->xheaders['X-Mailer'] = "Php/libMail2008v1.7.0";
