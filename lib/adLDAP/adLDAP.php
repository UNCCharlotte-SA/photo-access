 ...
 **********************************************************************************************************************
*
* Updater: Bach Nguyen (bnguye21@uncc.edu)
* Last Updated: 11/18/2013
* Updated: 
*     function connect()
*     function authenticate()
*         Changed all return function into array of result => display back to user
*
 ***********************************************************************************************************************
 */
...
class adLDAP {
    ...
    */
    public function connect() 
    {
        ...
        if ($this->useTLS) {
            $ldap_start_tls_boolean = ldap_start_tls($this->ldapConnection);
			if ($ldap_start_tls_boolean == false) { // 2014-10-15 - Bach Nguyen added the else statement
				$connectStatus['status'] = false;
				$connectStatus['error'] = "Error 3: ".$this->getLastError();
        ...
        if ($this->adminUsername !== NULL && $this->adminPassword !== NULL) {
            ...
            if (!$this->ldapBind) {
                if ($this->useSSL && !$this->useTLS) {
					...
					$connectStatus['status'] = false;
					$connectStatus['error'] = "Error 1: ".$this->getLastError();
					return $connectStatus;
                }
                else {
					...
					$connectStatus['status'] = false;
					$connectStatus['error'] = "Error 2: ".$this->getLastError();
					return $connectStatus;
                }
            }
        }
    ...
    */
    public function authenticate($username, $password, $preventRebind = false) {
		...
		$authenticateStatus = array();
		$authenticateStatus['status'] = "";
		$authenticateStatus['error'] = "";
        //if ($username === NULL || $password === NULL) { return false; } 
        //if (empty($username) || empty($password)) { return false; }
		if (empty($username) || ($username === NULL)) {
			$authenticateStatus['status'] = false;
			$authenticateStatus['error'] = "Error 4: Missing username";
			return $authenticateStatus;
		}
		
		if (empty($password) || ($password === NULL)) {
			$authenticateStatus['status'] = false;
			$authenticateStatus['error'] = "Error 5: Missing Password";
			return $authenticateStatus;
		}
        ...
        if ($this->useSSO && $_SERVER['REMOTE_USER'] && $_SERVER['REMOTE_USER'] == $username && $this->adminUsername === NULL && $_SERVER['KRB5CCNAME']) { 
            ...
            if (!$this->ldapBind) {
                //throw new adLDAPException('Rebind to Active Directory failed. AD said: ' . $this->getLastError());
				$authenticateStatus['status'] = false;
				$authenticateStatus['error'] = "Error 6: ".$this->getLastError();
				return $authenticateStatus;
            }
            else {
                //return true;
				$authenticateStatus['status'] = true;
				$authenticateStatus['error'] = "";
				return $authenticateStatus;
            }
        }
        ...
        if (!$this->ldapBind){ 
            //$ret = false; 
			$authenticateStatus['status'] = false;
			//$authenticateStatus['error'] = "Error 7: ".$this->getLastError();
			$authenticateStatus['error'] = "Error 7: username/password pair not found";
			return $authenticateStatus;
        }
        ...
        if ($this->adminUsername !== NULL && !$preventRebind) {
            ...
            if (!$this->ldapBind){
                ...
                //throw new adLDAPException('Rebind to Active Directory failed. AD said: ' . $this->getLastError());
				$authenticateStatus['status'] = false;
				$authenticateStatus['error'] = "Error 8: ".$this->getLastError();
				return $authenticateStatus;
            } 
        } 
