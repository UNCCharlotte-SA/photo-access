...
class Zebra_Database
{
    ...
    function connect($host, $user, $password, $database, $port = '', $socket = '', $connect = false)
    {
        ...
        if ($connect) {
			$connectStatus = $this->_connected();
			print_r($connectStatus);
    ...
    function escape($string)
    {
        ...
        if ($this->_connected()['status'] == "true") {
    ...
    function check_connect()
    {
        ...
        if ($this->_connected()['status'] == "true") {
			...
			$connectStatus['message'] = $this->language['successful_queries'];
			$connectStatus['error'] = "";
			return $connectStatus;
		} else {
			$connectStatus['status'] = "false";
			$connectStatus['message'] = $this->language['could_not_connect_to_database'];
			$connectStatus['error'] = $this->_connected();
			return $connectStatus;
		}
    ...
    function get_link()
    {
        ...
        if ($this->_connected()['status'] == "true") return $this->connection;
    ...
    */
    public function get_selected_database() {
        ...
        // if an active connection exists
        if ($this->_connected()['status'] == "true") return $this->credentials['database'];
    ...
    private function _connected()
    {
        ...
        if (!$this->connection) {
            ...
            if (mysqli_connect_errno()) {
				...
				$connectStatus['status'] = "false";
				$connectStatus['message'] = $this->language['could_not_connect_to_database'];
				$connectStatus['error'] = mysqli_connect_error();
                $this->_log('errors', array(
            ...
            if (!$this->memcache && $this->memcache_host !== false && $this->memcache_port !== false) {
                ...
                if (class_exists('Memcache')) {
                    ...
                    if (!$memcache->connect($this->memcache_host, $this->memcache_port)) {
						...
						$connectStatus['status'] = "false";
						$connectStatus['message'] = $this->language['could_not_connect_to_memcache_server'];
						$connectStatus['error'] = $this->language['could_not_connect_to_memcache_server'];;
                ...
                // if memcache extension is not installed
					...
					$connectStatus['status'] = "false";
					$connectStatus['message'] = $this->language['memcache_extension_not_installed'];
					$connectStatus['error'] = $this->language['could_not_connect_to_memcache_server'];;
				}
            }
